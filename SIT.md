...
半角の数字 4 桁を示しています。  
#### ([－ー―-]| - )
全角もしくは半角のハイフンとハイフンに類似する全角文字、注意事項 3 に留意した変換後の" - "のいずれかを示しています。  
#### [5789]
半角の 5,7,8,9 のいずれか一文字を示しています。  
#### \w
[A-Za-z0-9] と同等で半角英数1文字を示しています。  
#### (?<!\w|[－ー―-]|- )
他の部分がマッチした後、前に半角の英数字やハイフンがないことを確認する正規表現のアサーションとなっており、電話番号の形式を部分的に含む、E03-0000-0000 のようなシリアル コードなどの類似する文字列を排除するようにしています。  
#### (?!\w|[－ー―-]| -)
他の部分がマッチした後、後ろに半角の英数字やハイフンがないことを確認する正規表現のアサーションとなっており、電話番号の形式を部分的に含む、03-0000-0000-0000 のようなシリアル コードなどの類似する文字列を排除するようにしています。
...
西暦二〇〇〇年拾月吉日  
## 正規表現
    (明治|大正|昭和|平成|令和|西暦)[ |　]*([\d{1,4}|[元一二三四五六七八九十壱弐参拾〇○零]{1,4})[ |　]*年[ |　]*([\d{1,2}|[一二三四五六七八九十壱弐参拾〇○]{1,2})[ |　]*月[ |　]*([\d{1,2}|[元一二三四五六七八九十壱弐参拾〇○吉]{1,2})[ |　]*日
